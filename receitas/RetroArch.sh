...
APP_DIR="$HOME/.local/share/ospkg/apps/RetroArch"
DESKTOP_DIR="$HOME/.local/share/applications"
UNINSTALL_DIR="/usr/share/ospkg/app-uninstall"

echo "-> [OSPන්K] Iniciando a instalação do RetroArch..."
...
DEST_FILE="$APP_DIR/RetroArch.AppImage"

echo "-> Baixando o binário do RetroArch..."
curl -L -o "$DEST_FILE" "$APPIMAGE_URL"

if [ ! -f "$DEST_FILE" ]; then
    echo "Erro: Falha ao baixar o RetroArch."
    exit 1
fi
...
cat << EOF > "$DESKTOP_DIR/RetroArch.desktop"
[Desktop Entry]
Name=RetroArch
Exec=$DEST_FILE
...
sudo tee "$UNINSTALL_DIR/RetroArch.sh" > /dev/null << 'EOF'
#!/bin/bash
echo "-> Removendo o RetroArch..."
rm -rf "$HOME/.local/share/ospkg/apps/RetroArch"
rm -f "$HOME/.local/share/applications/RetroArch.desktop"
echo "-> RetroArch desinstalado com sucesso!"
EOF

sudo chmod +x "$UNINSTALL_DIR/RetroArch.sh"

echo "-> RetroArch instalado com sucesso! Já aparece no seu menu de apps."
