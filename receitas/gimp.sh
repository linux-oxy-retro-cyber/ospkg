...
APP_DIR="$HOME/.local/share/ospkg/apps/gimp"
DESKTOP_DIR="$HOME/.local/share/applications"
UNINSTALL_DIR="/usr/share/ospkg/app-uninstall"

echo "-> [OSPන්K] Iniciando a instalação do gimp..."
...
if [ ! -f "$DEST_FILE" ]; then
    echo "Erro: Falha ao baixar o gimp."
    exit 1
fi
...
cat << EOF > "$DESKTOP_DIR/gimp.desktop"
[Desktop Entry]
Name=GIMP
Exec=$DEST_FILE
...
sudo tee "$UNINSTALL_DIR/gimp.sh" > /dev/null << 'EOF'
#!/bin/bash
echo "-> Removendo o gimp..."
rm -rf "$HOME/.local/share/ospkg/apps/gimp"
rm -f "$HOME/.local/share/applications/gimp.desktop"
echo "-> gimp desinstalado com sucesso!"
EOF

sudo chmod +x "$UNINSTALL_DIR/gimp.sh"

echo "-> gimp instalado com sucesso! Já aparece no seu menu de apps."
