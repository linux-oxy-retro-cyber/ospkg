...
APP_DIR="$HOME/.local/share/ospkg/apps/vlc"
DESKTOP_DIR="$HOME/.local/share/applications"
UNINSTALL_DIR="/usr/share/ospkg/app-uninstall"

echo "-> [OSPන්K] Iniciando a instalação do VLC..."
...
DEST_FILE="$APP_DIR/vlc.AppImage"

echo "-> Baixando o binário do VLC..."
curl -L -o "$DEST_FILE" "$APPIMAGE_URL"

if [ ! -f "$DEST_FILE" ]; then
    echo "Erro: Falha ao baixar o VLC."
    exit 1
fi
...
cat << EOF > "$DESKTOP_DIR/vlc.desktop"
[Desktop Entry]
...
sudo tee "$UNINSTALL_DIR/vlc.sh" > /dev/null << 'EOF'
#!/bin/bash
echo "-> Removendo o VLC..."
rm -rf "$HOME/.local/share/ospkg/apps/vlc"
rm -f "$HOME/.local/share/applications/vlc.desktop"
echo "-> VLC desinstalado com sucesso!"
EOF

sudo chmod +x "$UNINSTALL_DIR/vlc.sh"

echo "-> VLC instalado com sucesso! Já aparece no seu menu de apps."
